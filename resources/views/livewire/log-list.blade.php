<div
    class="h-full w-full py-5 log-list"
    @if($refreshAutomatically) wire:poll @endif
    x-cloak
    x-data
    ...
    <div class="flex flex-col h-full w-full mx-3 mb-4">
        ...
        @isset($selectedFileIdentifier)
            <div class="relative overflow-hidden text-sm h-full" x-data x-init="$nextTick(() => { })">
                <div
                    id="log-item-container"
                    class="log-item-container h-full overflow-y-auto px-4"
                    x-on:scroll="(event) => $store.logViewer.onScroll(event)"
                >
                    <div class="inline-block min-w-full max-w-full align-middle">
                        <table
                            wire:key="{{ \Illuminate\Support\Str::random(16) }}"
                            class="table-fixed min-w-full max-w-full border-separate border-spacing-0"
                            x-init="$nextTick(() => { $store.logViewer.reset(); if ({{ $expandAutomatically ? 'true' : 'false' }}) { $store.logViewer.stacksOpen.push(0) } })"
                        >
                            <thead class="bg-gray-50">
                            <tr>
                                <th scope="col" class="w-[60px] pl-4 pr-2 sm:pl-6 lg:pl-8">
                                    ...
                                    <div class="flex justify-between">
                                        ...
                                        <div>
                                            ...
                                            <select
                                                ...
                                                class="bg-gray-100 dark:bg-gray-900 px-2 font-normal mr-3 outline-none rounded focus:ring-2 focus:ring-emerald-500 dark:focus:ring-emerald-600"
                                            >
                                            ...
                                            <select
                                                id="items-per-page"
                                                wire:model="perPage"
                                                class="bg-gray-100 dark:bg-gray-900 px-2 font-normal outline-none rounded focus:ring-2 focus:ring-emerald-500 dark:focus:ring-emerald-600"
                                            >
                            ...
                            @forelse($logs as $index => $log)
                                <tbody
                                    class="log-group @if($index === 0) first @endif"
                                    id="tbody-{{$index}}"
                                    data-index="{{ $index }}"
                                >
                                <tr
                                    class="log-item {{ $log->level->getClass() }}"
                                ...
                                <tr x-show="$store.logViewer.isOpen({{$index}})">
                                    <td colspan="6">
                                        ...
                                        @if($log->fullTextIncomplete)
                                            <div class="py-4 px-8 text-gray-500 italic">
                                                The contents of this log have been cut short to the
                                                first {{ bytes_formatted(\Opcodes\LogViewer\Facades\LogViewer::maxLogSize()) }}
                                                .
                                                The full size of this log entry is
                                                <strong>{{ $log->fullTextLengthFormatted() }}</strong>
                                            </div>
                                        @endif
                                    </td>
                                </tr>
