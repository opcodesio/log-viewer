    ...
    <div class="h-full overflow-y-scroll py-6">
        @foreach($files as $logFile)
            <div wire:click="selectFile('{{ $logFile->name }}')"
                 class="mb-2 text-gray-800 rounded-md bg-white overflow-hidden transition duration-100 border-2 border-transparent hover:border-emerald-600 cursor-pointer @if($file === $logFile->name) border-r-[4px] border-emerald-500 @endif">
                <div
                    class="flex justify-between items-center px-[12px] py-2 @if($file === $logFile->name) pr-[10px] @endif">
