<nav class=" border-gray-200 mt-2 px-4 w-full flex items-center justify-between sm:px-0">
    <div class="-mt-px w-0 flex-1 flex" wire:key="pagination-previous-{{ $paginator->currentPage() }}">
        ...
        @else
        <button wire:click="previousPage" wire:loading.attr="disabled" rel="prev" class="border-t-2 border-transparent pt-4 pr-1 inline-flex items-center text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300">
            <!-- Heroicon name: solid/arrow-narrow-left -->
            <svg class="mr-3 h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                <path fill-rule="evenodd" d="M7.707 14.707a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l2.293 2.293a1 1 0 010 1.414z" clip-rule="evenodd" />
            </svg>
    ...
    <div class="hidden md:-mt-px md:flex" wire:key="pagination-page-{{ $paginator->currentPage() }}">
        ...
        @foreach($links as $link)
            @if($link['active'])
                <button class="border-indigo-500 text-indigo-600 border-t-2 pt-4 px-4 inline-flex items-center text-sm font-medium" aria-current="page">
